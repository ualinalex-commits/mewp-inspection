  ...
  for (const { day, entry, defects } of blocks) {
    ...
    if (faultLines > 0) {
      ...
      for (let ri = 0; ri < defects.length; ri++) {
        ...
        const ROW_Y    = y - TABLE_ROW_H + 4;
        const rawDesc  = (ITEM_DESC[d.item_number] || '').replace(/^\d+\s+/, '');

        page.drawText(String(d.item_number),
          { x: COL_ITEM_X   + 2, y: ROW_Y, size: 6.5, font,        color: BLACK });
        page.drawText(rawDesc.substring(0, 38),
          { x: COL_DESC_X   + 2, y: ROW_Y, size: 6,   font,        color: BLACK });
        page.drawText((d.defect_details || '—').substring(0, 58),
          { x: COL_DETAIL_X + 2, y: ROW_Y, size: 6,   font,        color: BLACK });
        ...
        page.drawText(stText,
          { x: COL_STATUS_X + 2, y: ROW_Y, size: 6.5, font: bold,  color: stCol });
