...
async function generateReport(mewpId, weekCommencing) {
  ...
  const { data: entryRows = [] } = await supabase
    ...
    .select('inspection_date, day_of_week, operator_name, pal_card_number, daily_status, created_at')
    .eq('sheet_id', sheet.id);
  ...
  (entryRows || []).forEach(row => {
    ...
    if (!operatorsByDay[day]) {
      ...
      if (!operatorsByDay[day].created_at)      operatorsByDay[day].created_at      = row.created_at;
    }
  });
