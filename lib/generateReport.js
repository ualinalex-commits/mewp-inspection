...
// Operator log rows on page 2 — x positions shared across all days
const OP_NAME_X   = 153;
const OP_STATUS_X = 256;
// Y positions extracted from template.pdf via extractCoords.mjs
const DAY_OP_Y = { Mon: 690, Tue: 666, Wed: 640, Thu: 615, Fri: 591, Sat: 567, Sun: 541 };
...
async function buildPDF({ mewp, sheet, summaryByItem, operatorsByDay }) {
  ...
  // ── Page 0: header ──────────────────────────────────────────────────────────
  const machineLabel = mewp.serial_number ? `${mewp.machine_ref || ''} - ${mewp.serial_number}` : (mewp.machine_ref || '');
  ...
  // ── Page 1: initials ────────────────────────────────────────────────────────
  // "Initialled:" label is at y=545; stamp boxes sit at y=531 (confirmed via pdfjs-dist)
  for (const day of DAYS) {
  ...
  // ── Page 2: header ──────────────────────────────────────────────────────────
  stamp(p3, machineLabel, MACHINE_REF_NO.x, MACHINE_REF_NO.y, 8, BLACK, false);

  // ── Page 2: operator rows (all 7 days) ─────────────────────────────────────
  for (const day of DAYS) {
    const op = operatorsByDay[day] || {};
    const y  = DAY_OP_Y[day];
    stamp(p3, op.operator_name || '', OP_NAME_X, y);
    stamp(p3, op.daily_status === 'ok' ? 'OK' : op.daily_status === 'fault' ? 'X' : '', OP_STATUS_X, y);
  }
...
async function generateReport(mewpId, weekCommencing) {
  ...
  // Fallback: if the view didn't include operator_name, pull it from daily_inspection_entries directly
  const missingName = DAYS.some(d => operatorsByDay[d] && !operatorsByDay[d].operator_name);
  const noEntries   = DAYS.every(d => !operatorsByDay[d]);
  if (missingName || noEntries) {
    const { data: entryRows = [] } = await supabase
      .from('daily_inspection_entries')
      .select('inspection_date, day_of_week, operator_name, daily_status')
      .eq('sheet_id', sheet.id);

    (entryRows || []).forEach(row => {
      const day = normDay(row.day_of_week) || dayFromDate(row.inspection_date);
      if (!day) return;
      if (!operatorsByDay[day]) operatorsByDay[day] = row;
      else if (!operatorsByDay[day].operator_name) operatorsByDay[day].operator_name = row.operator_name;
    });
  }
  ...
  // Delete before uploading so the CDN doesn't serve a stale cached version.
