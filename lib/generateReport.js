...
async function generateReport(mewpId, weekCommencing) {
  ...
  // 4. daily_inspection_entries — query the full week by mewp_id + date range.
  // Avoids relying on sheet_id (which may differ per-day if sheets were created
  // separately) and guarantees every submitted day in the week is included.
  // Direct table rows always override anything the view populated above.
  ...
  // 5. defect_log — query by mewp_id + date range so all week's faults are fresh
