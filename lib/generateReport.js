...
// Mon x shifts at item 10 (two-digit item numbers nudge the column 3px left)
function visMonX(item) { return item <= 9 ? 442 : 439; }
function visSatX(item) { return item <= 9 ? 690 : 688; }

// Function checks 29-43: exact x per column, y per row, page index
const FUNC = {
  29: { monGx: 435, monPx: 459, satGx: 681, satPx: 704, y: 223, page: 0 },
  30: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 206, page: 0 },
  31: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 189, page: 0 },
  32: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 173, page: 0 },
  33: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 156, page: 0 },
  34: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 139, page: 0 },
  35: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 123, page: 0 },
  36: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 106, page: 0 },
  37: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y:  89, page: 0 },
  38: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y:  73, page: 0 },
  39: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y:  56, page: 0 },
  40: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y:  39, page: 0 },
  41: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y:  23, page: 0 },
  42: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y:   6, page: 0 },
  43: { monGx: 435, monPx: 459, satGx: 681, satPx: 705, y: 562, page: 1 },
};

// Page 1 initials
const MON_INITIAL = { x: 436, y: 531 };  // {{mon_initial}}
const SAT_INITIAL = { x: 681, y: 531 };  // {{sat_initial}}
...
async function buildPDF({ mewp, sheet, summaryByItem, operatorsByDay }) {
  ...
  for (let num = 1; num <= 28; num++) {
    ...
    const y   = VIS_Y[num];
    const monR = toResult(row.mon_result  ?? row.mon  ?? null);
    const satR = toResult(row.sat_result  ?? row.sat  ?? null);
    if (monR) stampResult(p1, monR, visMonX(num), y);
    if (satR) stampResult(p1, satR, visSatX(num), y);
  }
  ...
  for (let num = 29; num <= 43; num++) {
    ...
    const monG = toResult(row.mon_ground_result ?? row.mon_ground ?? null);
    const monP = toResult(row.mon_platform_result ?? row.mon_platform ?? null);
    const satG = toResult(row.sat_ground_result ?? row.sat_ground ?? null);
    const satP = toResult(row.sat_platform_result ?? row.sat_platform ?? null);
    if (monG) stampResult(pg, monG, fc.monGx, fc.y);
    if (monP) stampResult(pg, monP, fc.monPx, fc.y);
    if (satG) stampResult(pg, satG, fc.satGx, fc.y);
    if (satP) stampResult(pg, satP, fc.satPx, fc.y);
  }

  // ── Page 1: initials ────────────────────────────────────────────────────────
  const monOp = operatorsByDay['Mon'] || {};
  const satOp = operatorsByDay['Sat'] || {};
  if (monOp.operator_name) {
    stamp(p2, monOp.operator_name.trim()[0].toUpperCase(), MON_INITIAL.x, MON_INITIAL.y);
  }
  if (satOp.operator_name) {
    stamp(p2, satOp.operator_name.trim()[0].toUpperCase(), SAT_INITIAL.x, SAT_INITIAL.y);
  }
