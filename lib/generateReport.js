...
function appendDailySummaryPages(pdfDoc, font, bold, operatorsByDay, defectsByDay) {
  const PAGE_W    = 595.32;
  const PAGE_H    = 841.92;
  const MARGIN    = 30;
  ...
  const TABLE_ROW_H = 24;  // 2-line rows: description wraps to 2 × 6pt lines
  const DAY_HDR_H   = 22;
  const INFO_ROW_H  = 22;
  ...
  for (const { day, entry, defects } of blocks) {
    ...
    if (faultLines > 0) {
      ...
      for (let ri = 0; ri < defects.length; ri++) {
        ...
        // Vertical positions within 24pt row
        const MID_Y   = y - 12;  // single-line items (item number, status)
        const LINE1_Y = y - 8;   // upper text line when wrapping to 2 lines
        const LINE2_Y = y - 17;  // lower text line when wrapping to 2 lines

        // Item number
        page.drawText(String(d.item_number),
          { x: COL_ITEM_X + 2, y: MID_Y, size: 6.5, font: bold, color: BLACK });

        // Description — exact form question text, word-wrapped to 2 lines
        ...
        // Fault details — user-entered defect description, word-wrapped to 2 lines
...
async function buildPDF({ mewp, sheet, summaryByItem, operatorsByDay, defectsByDay }) {
  ...
  // pdfDoc currently has pages [0, 1, 2]; remove index 2 before adding new ones.
  pdfDoc.removePage(2);

  // ── Dynamic daily summary pages ─────────────────────────────────────────────
  appendDailySummaryPages(pdfDoc, font, bold, operatorsByDay, defectsByDay || {});
...
async function generateReport(mewpId, weekCommencing) {
  ...
  // 4. daily_inspection_entries — enrich with PAL card and inspection time
  // Use submitted_at (explicitly inserted) not created_at (not in schema)
  const { data: entryRows = [], error: entryErr } = await supabase
    .from('daily_inspection_entries')
    .select('inspection_date, day_of_week, operator_name, pal_card_number, daily_status, submitted_at')
    .eq('sheet_id', sheet.id);
  ...
  (entryRows || []).forEach(row => {
    const day = normDay(row.day_of_week) || dayFromDate(row.inspection_date);
    if (!day) return;
    if (!operatorsByDay[day]) {
      operatorsByDay[day] = row;
    } else {
      if (!operatorsByDay[day].operator_name)   operatorsByDay[day].operator_name   = row.operator_name;
      if (!operatorsByDay[day].pal_card_number)  operatorsByDay[day].pal_card_number  = row.pal_card_number;
      if (!operatorsByDay[day].inspection_date)  operatorsByDay[day].inspection_date  = row.inspection_date;
      if (!operatorsByDay[day].submitted_at)     operatorsByDay[day].submitted_at     = row.submitted_at;
      if (!operatorsByDay[day].daily_status)     operatorsByDay[day].daily_status     = row.daily_status;
    }
  });

  // 5. defect_log — grouped by day
  ...
  // 6. Build PDF
  const pdfBuffer = await buildPDF({ mewp, sheet, summaryByItem, operatorsByDay, defectsByDay });
