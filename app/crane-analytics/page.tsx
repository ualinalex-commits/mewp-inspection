...
function CraneAnalyticsContent() {
  ...
  const downloadDailyPDF = useCallback(async (selectedDate: string) => {
    setDailyReportLoading(true);
    try {
      const { jsPDF } = await import('jspdf');

      // ─── Data ────────────────────────────────────────────────────────
      const dayAll = rows.filter(r =>
        r.date === selectedDate &&
        (!effectiveSite || r.site === effectiveSite) &&
        (!crane || r.crane === crane)
      );
      const dayTimed = dayAll.filter(r => r.start_time && r.end_time);

      if (dayAll.length === 0) {
        alert(`No logs found for ${fmtDateLong(selectedDate)}.`);
        return;
      }
      ...
      const craneCounts: Record<string, number> = {};
      for (const r of dayTimed) {
        const m = parseMins(r.start_time, r.end_time);
        totalMins += m;
        craneCounts[r.crane] = (craneCounts[r.crane] ?? 0) + 1;
        if (!firstLog || r.start_time < firstLog) firstLog = r.start_time;
        if (!lastLog  || r.end_time   > lastLog)  lastLog  = r.end_time;
      }

      const craneMap: Record<string, { logs: number; mins: number; first: string | null; last: string | null }> = {};
      for (const r of dayAll) {
        if (!craneMap[r.crane]) craneMap[r.crane] = { logs: 0, mins: 0, first: null, last: null };
        craneMap[r.crane].logs++;
        if (r.start_time && r.end_time) {
          craneMap[r.crane].mins += parseMins(r.start_time, r.end_time);
          if (!craneMap[r.crane].first || r.start_time < craneMap[r.crane].first!) craneMap[r.crane].first = r.start_time;
          if (!craneMap[r.crane].last  || r.end_time   > craneMap[r.crane].last!)  craneMap[r.crane].last  = r.end_time;
        }
      }
      const craneEntries = Object.entries(craneMap).sort(([a], [b]) => a.localeCompare(b));

      // Idle per crane, respecting per-crane overtime
      let totalIdleMins = 0;
      const craneIdleMap: Record<string, { idle: number; craneWin: number }> = {};
      for (const [cn, cv] of craneEntries) {
        const maxEnd = dayTimed.filter(r => r.crane === cn).reduce((mx, r) => Math.max(mx, dtToTimeMins(r.end_time)), dayEndMins);
        const craneWin = Math.max(dayEndMins, maxEnd) - dayStartMins;
        const idle = Math.max(0, craneWin - cv.mins);
        craneIdleMap[cn] = { idle, craneWin };
        totalIdleMins += idle;
      }

      const topCrane = Object.entries(craneCounts).sort(([, a], [, b]) => b - a)[0]?.[0] ?? '—';
      const cranesActive = craneEntries.length;
      const totalWin = craneEntries.reduce((s, [cn]) => s + craneIdleMap[cn].craneWin, 0);
      const totalIdlePct = totalWin > 0 ? Math.round((totalIdleMins / totalWin) * 100) : 0;

      const companyMap: Record<string, number> = {};
      for (const r of dayTimed) {
        const k = r.company?.trim(); if (!k) continue;
        companyMap[k] = (companyMap[k] ?? 0) + parseMins(r.start_time, r.end_time);
      }
      const companyEntries = Object.entries(companyMap).sort(([, a], [, b]) => b - a);
      const companyTotal = companyEntries.reduce((s, [, v]) => s + v, 0);

      const statusMap: Record<string, number> = {};
      for (const r of dayTimed) {
        const k = r.status?.trim() || 'Unknown';
        statusMap[k] = (statusMap[k] ?? 0) + parseMins(r.start_time, r.end_time);
      }
      const statusEntries = Object.entries(statusMap).sort(([, a], [, b]) => b - a);
      const statusTotal = statusEntries.reduce((s, [, v]) => s + v, 0);

      const logRows = [...dayAll].sort((a, b) => (a.start_time ?? '') < (b.start_time ?? '') ? -1 : 1);
      ...
      const mg = 14;
      const cW = PW - mg * 2;
      const bottomEdge = PH - mg - 10;

      const clip = (s: string, colW: number): string => {
        const maxChars = Math.floor(colW / 1.85);
      ...
      const addPageNums = () => {
        ...
        for (let p = 1; p <= total; p++) {
          ...
          const t = `Page ${p} of ${total}`;
          doc.text(t, (PW - doc.getTextWidth(t)) / 2, PH - 5);
        }
      };

      const secLabel = (label: string, y: number): number => {
        doc.setFontSize(7.5); doc.setFont('helvetica', 'bold'); doc.setTextColor('#6b7280');
        doc.text(label, mg, y);
        return y + 6;
      };

      const tblHdr = (hs: string[], ws: number[], x0: number, y0: number): number => {
        doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.setTextColor('#6b7280');
        let x = x0;
        hs.forEach((h, i) => { doc.text(h.toUpperCase(), x, y0); x += ws[i]; });
        const ly = y0 + 2.5;
        doc.setDrawColor('#e5e7eb'); doc.setLineWidth(0.3);
        doc.line(x0, ly, x0 + ws.reduce((a, b) => a + b, 0), ly);
        return ly + 4;
      };

      const drawSep = (y: number): number => {
        doc.setDrawColor('#e5e7eb'); doc.setLineWidth(0.4);
        doc.line(mg, y, PW - mg, y);
        return y + 6;
      };

      // ─── Page 1: Cover + Summary + Crane Breakdown ───────────────────
      doc.setFillColor(208, 42, 53);
      doc.rect(0, 0, PW, 56, 'F');
      doc.setFillColor(255, 255, 255);
      doc.rect(0, 55.5, PW, 0.5, 'F');

      doc.setTextColor('#ffffff');
      doc.setFont('helvetica', 'bold'); doc.setFontSize(22);
      doc.text('Daily Crane Operations Report', mg, 20);

      doc.setFont('helvetica', 'normal'); doc.setFontSize(11);
      doc.text(effectiveSite || 'All Sites', mg, 32);

      doc.setFont('helvetica', 'bold'); doc.setFontSize(13);
      doc.text(fmtDateLong(selectedDate), mg, 44);

      doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor('#fca5a5');
      const bTxt = 'ProLifting Software';
      doc.text(bTxt, PW - mg - doc.getTextWidth(bTxt), 10);
      const genTxt = `Generated ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}`;
      doc.text(genTxt, PW - mg - doc.getTextWidth(genTxt), 52);
      ...
      const sumStats = [
        { l: 'Total Logs',        v: String(dayAll.length) },
        { l: 'Working Time',      v: fmtMins(totalMins) },
        { l: 'Total Idle',        v: fmtMins(totalIdleMins) },
        { l: 'Idle %',            v: `${totalIdlePct}%` },
        { l: 'First Log',         v: fmtTime(firstLog) },
        { l: 'Last Log',          v: fmtTime(lastLog) },
        { l: 'Most Active Crane', v: topCrane },
        { l: 'Cranes Active',     v: String(cranesActive) },
      ];
      const scW = cW / sumStats.length;
      for (let i = 0; i < sumStats.length; i++) {
        const sx = mg + i * scW;
        doc.setFontSize(12); doc.setFont('helvetica', 'bold'); doc.setTextColor('#111827');
        doc.text(clip(sumStats[i].v, scW - 2), sx, y);
        doc.setFontSize(6.5); doc.setFont('helvetica', 'normal'); doc.setTextColor('#6b7280');
        doc.text(sumStats[i].l.toUpperCase(), sx, y + 4.5);
      }
      y += 15;
      y = drawSep(y);

      // Crane breakdown
      y = secLabel('CRANE BREAKDOWN', y);
      const cbH = ['Crane', 'Total Logs', 'Working', 'Idle', 'Idle %', 'First Log', 'Last Log'];
      const cbW = [74, 28, 40, 35, 28, 32, 32];
      y = tblHdr(cbH, cbW, mg, y);
      ...
      for (const [cn, cv] of craneEntries) {
        ...
        const { idle: crIdle, craneWin } = craneIdleMap[cn];
        const idlePct = craneWin > 0 ? Math.round((crIdle / craneWin) * 100) : 0;
        const cells = [cn, String(cv.logs), fmtMins(cv.mins), fmtMins(crIdle), `${idlePct}%`, fmtTime(cv.first), fmtTime(cv.last)];
        let x = mg;
        cells.forEach((cell, i) => { doc.text(clip(cell, cbW[i] - 2), x, y); x += cbW[i]; });
        y += 6.5;
      }

      // ─── Page 2: Allocations + Logs ──────────────────────────────────
      doc.addPage(); y = mg;

      // By company
      y = secLabel('TIME ALLOCATION BY COMPANY', y);
      const tcH = ['Company', 'Working Time', 'Share'];
      const tcW = [159, 60, 50];
      y = tblHdr(tcH, tcW, mg, y);
      doc.setFontSize(8.5); doc.setFont('helvetica', 'normal'); doc.setTextColor('#374151');
      for (const [company, mins] of companyEntries) {
        if (y > bottomEdge) { doc.addPage(); y = mg; y = tblHdr(tcH, tcW, mg, y); doc.setFontSize(8.5); doc.setFont('helvetica', 'normal'); doc.setTextColor('#374151'); }
        const pct = companyTotal > 0 ? Math.round((mins / companyTotal) * 100) : 0;
        let x = mg;
        [company, fmtMins(mins), `${pct}%`].forEach((cell, i) => { doc.text(clip(cell, tcW[i] - 2), x, y); x += tcW[i]; });
        y += 6.5;
      }
      y += 5;

      // By status
      y = secLabel('TIME ALLOCATION BY STATUS', y);
      const tsH = ['Status', 'Working Time', 'Share'];
      const tsW = [159, 60, 50];
      y = tblHdr(tsH, tsW, mg, y);
      doc.setFontSize(8.5); doc.setFont('helvetica', 'normal'); doc.setTextColor('#374151');
      for (const [status, mins] of statusEntries) {
        if (y > bottomEdge) { doc.addPage(); y = mg; y = tblHdr(tsH, tsW, mg, y); doc.setFontSize(8.5); doc.setFont('helvetica', 'normal'); doc.setTextColor('#374151'); }
        const pct = statusTotal > 0 ? Math.round((mins / statusTotal) * 100) : 0;
        let x = mg;
        [status, fmtMins(mins), `${pct}%`].forEach((cell, i) => { doc.text(clip(cell, tsW[i] - 2), x, y); x += tsW[i]; });
        y += 6.5;
      }
      y += 5;

      if (y + 10 < bottomEdge) { y = drawSep(y); } else { doc.addPage(); y = mg; }

      // Full log entries
      y = secLabel(`ALL LOG ENTRIES — ${fmtDateLong(selectedDate).toUpperCase()}`, y);
      const lgH = ['Time', 'Crane', 'Supervisor', 'Company', 'Load Description', 'Status', 'Duration'];
      const lgW = [36, 38, 38, 38, 67, 27, 25];
      y = tblHdr(lgH, lgW, mg, y);
      ...
      for (const r of logRows) {
        ...
        const timeStr = r.start_time ? `${fmtTime(r.start_time)}–${fmtTime(r.end_time)}` : '—';
        const dur = r.start_time && r.end_time ? fmtMins(parseMins(r.start_time, r.end_time)) : '—';
        let x = mg;
        [timeStr, r.crane || '—', r.supervisor_name || '—', r.company || '—', r.load_description || '—', r.status || '—', dur]
          .forEach((cell, i) => { doc.text(clip(cell, lgW[i] - 2), x, y); x += lgW[i]; });
        y += 5.5;
      }
      ...
      const safeSite = (effectiveSite || 'all-sites').toLowerCase().replace(/[^a-z0-9]+/g, '-');
      doc.save(`daily-report-${safeSite}-${selectedDate}.pdf`);
    } catch (err) {
      console.error('Daily PDF failed:', err);
    } finally {
      setDailyReportLoading(false);
    }
  }, [rows, effectiveSite, crane, dayStartMins, dayEndMins]);
